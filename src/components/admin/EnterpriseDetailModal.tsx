// Phase ENT-OS-2 — 관리자 본사 통합 운영 콘솔 (Enterprise OS).
// 탭: 개요 / 브랜드(=Workspace) / 매장 / 프랜차이즈 음악 / 계약·정산 / 로그.
//  · 브랜드 탭 = Brand Workspace (Overview/Music/Playlist/Media/Player/AI/History 통합).
//    이전(ENT-OS-1)의 분산된 플레이어/이미지/브랜드 음악 탭을 Workspace 하나로 흡수.
//  · 기존 admin_get_enterprise_detail(0406) = 개요/매장/프랜차이즈 음악/계약·정산/로그 (무변경).
//  · admin_get_enterprise_os_detail(0408) = 브랜드 연결 여부/식별 (무변경).
//  · admin_get_brand_workspace(0409) = Workspace 내부 (BrandWorkspace 컴포넌트가 조회).
...
import {
  ...
  type EnterpriseDetail, type EntDetailStore, type EnterpriseOsDetail,
} from '@/lib/api/enterpriseDetailApi';
import { adminCreateBrand } from '@/lib/api/brandPlayerApi';
import BrandWorkspace from '@/components/admin/BrandWorkspace';

type DetailTab = 'overview' | 'brand' | 'stores' | 'music' | 'billing' | 'logs';
const TABS: ReadonlyArray<{ key: DetailTab; label: string }> = [
  { key: 'overview', label: '개요' },
  { key: 'brand', label: '브랜드' },
  { key: 'stores', label: '매장' },
  { key: 'music', label: '프랜차이즈 음악' },
...
// ── 브랜드 (Workspace) ───────────────────────────────────────────────
// 연결 브랜드가 있으면 Brand Workspace, 없으면 이 본사에 브랜드 생성 (코드 없음, 본사명 기본).
function BrandTab({ os, enterpriseId, enterpriseName, reload }: {
  ...
  return <BrandWorkspace brandId={brand.id} onBrandChanged={() => void reload()} />;
}
...
// ── 프랜차이즈 음악 (enterprise-level) ───────────────────────────────
// 브랜드 음악 정책은 브랜드 Workspace 로 이동. 여기는 프랜차이즈(본사) 정책만.
function MusicTab({ d }: { d: EnterpriseDetail }) {
  return (
    <AdminCard title={`프랜차이즈 음악 정책 (${d.music_policy.length})`} subtitle="본사(franchise) 단위 정책 — 매장 정책 버전의 소스. 브랜드 음악 정책은 '브랜드' 탭 Workspace 에서.">
