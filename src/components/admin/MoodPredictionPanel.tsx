...
import {
  listMoodPredictions,
  applyAiMoodToTrack,
  getMoodClassificationStatus,
  type MoodPredictionRow,
  type MoodPredictionFilter,
  type MoodClassificationStatus,
} from '@/lib/moodPredictionApi';
import AutoCover from '@/components/AutoCover';
import { toast } from '@/store/toastStore';

/**
 * Phase X1.2 — CLAP zero-shot Mood 분류 결과 비교 / 1-click 적용.
 *
 * 적용 시 tracks.mood = top1 (name_ko), tracks.mood_tags = top1~3 (name_ko[]).
 * predicted_main_genre 은 보존됨 (X1.1 결과 영향 없음).
 */

const FILTERS: Array<{ key: MoodPredictionFilter; label: string }> = [
  { key: 'all', label: '전체' },
  { key: 'unapplied', label: '미적용' },
  { key: 'mismatch', label: '불일치' },
];

export default function MoodPredictionPanel() {
  const [filter, setFilter] = useState<MoodPredictionFilter>('all');
  const [rows, setRows] = useState<MoodPredictionRow[]>([]);
  const [status, setStatus] = useState<MoodClassificationStatus | null>(null);
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const [items, st] = await Promise.all([
        listMoodPredictions(filter, 50, 0),
        getMoodClassificationStatus(),
      ]);
      setRows(items);
      setStatus(st);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'mood 예측 조회 실패');
    } finally {
      setLoading(false);
    }
  }, [filter]);
  ...
  async function onApply(row: MoodPredictionRow) {
    if (!row.predicted_mood_labels || row.predicted_mood_labels.length === 0) {
      toast.error('예측 라벨 없음 (taxonomy slug 누락)');
      return;
    }
    setApplyingId(row.track_id);
    try {
      const res = await applyAiMoodToTrack(row.track_id);
      toast.success(`${res.previous_mood ?? '(빈 값)'} → ${res.applied_mood} (+태그 ${res.applied_mood_tags.length}) 적용`);
      await reload();
    } catch (err) {
    ...
    <div className="space-y-4">
      <header className="space-y-1">
        <h2 className="flex items-center gap-1.5 text-lg font-bold tracking-tight">
          <Sparkles size={16} className="text-accent" /> AI 무드 분류
        </h2>
        <p className="text-xs text-ink-mute">
          CLAP zero-shot 으로 추론된 무드 top1~3 을 artist 입력 메타와 비교하고 1-click 적용.
          적용 시 <code>tracks.mood</code> ← top1, <code>tracks.mood_tags</code> ← top1~3.
          모델: <span className="font-mono">taxonomy-v1</span> (genre 와 공존).
        </p>
      </header>
      ...
      <div className="rounded-2xl bg-bg-card p-4 ring-1 ring-line/10">
        {loading ? (
          ...
          <p className="py-8 text-center text-xs text-ink-dim">
            {filter === 'all' ? '아직 분류된 음원이 없습니다. Modal mood-backfill 을 실행해주세요.'
             : filter === 'unapplied' ? '미적용 예측이 없습니다.' : '불일치 예측이 없습니다.'}
          </p>
        ) : (
          <ul className="space-y-2">
            {rows.map((row) => (
              <MoodRow key={row.track_id} row={row} applying={applyingId === row.track_id} onApply={onApply} />
            ))}
          </ul>
        )}
      </div>
      ...
      <p className="text-[11px] leading-relaxed text-ink-dim">
        • <span className="font-semibold">필터</span> — 전체 / 미적용 / 불일치 (tracks.mood ≠ top1 label).<br />
        • <span className="font-semibold">적용</span> — top1 → <code>tracks.mood</code>, top1~3 → <code>tracks.mood_tags</code> 갱신.
          <code>predicted_main_genre</code> 등 genre 데이터는 보존됨.<br />
        • <span className="font-semibold">confidence</span> — top1 softmax 확률. 0.30 미만은 신중히 검토.
      </p>
    </div>
  );
}

function MoodRow({ row, applying, onApply }: {
  row: MoodPredictionRow;
  applying: boolean;
  onApply: (row: MoodPredictionRow) => void | Promise<void>;
}) {
  const conf = row.mood_confidence != null ? Number(row.mood_confidence) : null;
  const confTone =
    conf == null ? 'text-ink-dim'
    : conf >= 0.5 ? 'text-emerald-400'
    : conf >= 0.3 ? 'text-amber-400'
    : 'text-rose-400';
  const top1 = row.predicted_mood_labels?.[0] ?? row.predicted_mood_slugs?.[0] ?? null;
  const isMatch = !!row.current_mood && !!top1 && row.current_mood === top1;
  const isApplied = !!row.applied_at;

  const top3: Array<{ slug: string; label: string; score: number | null }> = [];
  const slugs = row.predicted_mood_slugs ?? [];
  const labels = row.predicted_mood_labels ?? [];
  const scores = row.prediction_scores?.mood;
    ...
    <li className="rounded-xl bg-bg-soft p-3 ring-1 ring-line/10">
      <div className="flex items-center gap-3">
        <div className="h-12 w-12 shrink-0 overflow-hidden rounded-lg ring-1 ring-line/10">
          <AutoCover title={row.title} category={row.current_mood ?? undefined} imageUrl={row.cover_url} size="sm" />
        </div>
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[11px]">
            ...
            <span className="rounded bg-bg-card px-1.5 py-0.5 font-mono text-[10px]">{row.current_mood ?? '(빈 값)'}</span>
        ...
        <button onClick={() => void onApply(row)}
          ...
          title={isMatch ? '이미 일치' : 'tracks.mood + mood_tags 갱신'}>
          <Check size={11} />
