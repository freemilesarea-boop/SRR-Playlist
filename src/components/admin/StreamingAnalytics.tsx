  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        ...
        {eligError && (
          <div className="rounded-lg bg-rose-500/25 px-3 py-2 text-xs text-slate-900 dark:text-rose-200 dark:text-red-300">
            {eligError}
          </div>
        )}
                  ...
                  {overview.map((d) => {
                    ...
                    return (
                      <tr key={d.day} className="border-b border-line/10 last:border-b-0">
                        ...
                        <td className="px-3 py-2 text-right tabular-nums font-semibold text-slate-900 dark:text-emerald-200 dark:text-emerald-300">{elig.toLocaleString()}</td>
        ...
        <div className="overflow-hidden rounded-2xl bg-bg-card ring-1 ring-line/10">
          ...
          {!eligLoading && breakdown && breakdown.total_excluded > 0 && (
            <div className="grid grid-cols-2 gap-2 p-3 sm:grid-cols-4 lg:grid-cols-8">
              ...
              <ReasonCard label="30초 중복" value={breakdown.dedup_30s_window} hint="같은 user/anon+track 30s 내 재생" tone="warn" />
...
function Summary({
  ...
  const valueClass =
    tone === 'success' ? 'text-slate-900 dark:text-emerald-200 dark:text-emerald-300'
    : tone === 'warn'  ? 'text-slate-900 dark:text-amber-200 dark:text-amber-300'
    : '';
  return (
...
function MonthlySettlementSummary() {
  ...
  return (
    <section className="space-y-3 rounded-2xl bg-gradient-to-br from-accent/10 to-accent-soft/5 p-4 ring-1 ring-accent/20">
      ...
      {error && (
        <div className="rounded-lg bg-rose-500/25 px-3 py-2 text-xs text-slate-900 dark:text-rose-200 dark:text-red-300">{error}</div>
      )}
...
function ReasonCard({
  ...
  const toneClass =
    tone === 'error' ? 'bg-rose-500/25 text-slate-900 dark:text-rose-200 ring-rose-400/50 dark:text-red-300'
    : tone === 'warn'  ? 'bg-amber-500/25 text-slate-900 dark:text-amber-200 ring-amber-400/50 dark:text-amber-300'
    : 'bg-sky-500/25 text-slate-900 dark:text-sky-200 ring-sky-400/50 dark:text-sky-300';
  return (
