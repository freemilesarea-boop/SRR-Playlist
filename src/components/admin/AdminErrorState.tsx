      ...
      {result && (
        <div
          className={`rounded-xl p-3 ring-1 ${
            result.ok ? 'bg-emerald-500/10 ring-emerald-500/30' : 'bg-red-500/10 ring-red-500/30'
          }`}
        >
          {result.ok ? (
            <div className="space-y-1 text-xs">
              <p className="flex items-center gap-1 font-bold text-emerald-200">
                <CheckCircle2 size={14} /> 분석 DB 적용 완료
              </p>
              <p className="text-emerald-100/85">{result.message}</p>
            ...
            <div className="space-y-1 text-xs">
              <p className="font-bold text-red-200">자동 적용 실패</p>
              <p className="font-mono text-red-300">{result.error}</p>
              {result.details && (
                <pre className="overflow-x-auto rounded bg-bg-deep/60 p-2 text-[10px] text-red-300/85">
                  {result.details}
                </pre>
              )}
              {result.hint && (
                <p className="rounded bg-yellow-500/10 p-2 text-yellow-200">💡 {result.hint}</p>
              )}
            </div>
          )}
        </div>
