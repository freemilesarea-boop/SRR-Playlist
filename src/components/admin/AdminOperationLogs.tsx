...
const LEVEL_META: Record<LogLevel, { label: string; tone: string; icon: React.ReactNode }> = {
  ...
  warning: { label: 'warning', tone: 'bg-yellow-500/25 text-slate-900 dark:text-yellow-200', icon: <AlertTriangle size={11} /> },
    ...
    <div className="space-y-4">
      <div className="flex items-baseline justify-between">
        ...
        <div className="flex gap-1">
          ...
          <button
            onClick={onClearOld}
            className="inline-flex items-center gap-1 rounded-md bg-rose-500/25 px-2 py-1 text-[11px] font-semibold text-slate-900 dark:text-rose-200 hover:bg-rose-500/35"
          >
            ...
            {rows.map((r) => {
              ...
              return (
                <li key={r.id} className="p-3 text-xs">
                  ...
                  {r.error_message && (
                    <p className="mt-1 rounded bg-red-100 px-2 py-1 font-mono text-[11px] text-slate-900 dark:text-rose-200 dark:bg-rose-500/20 dark:text-red-300">
