 ...
 *
 * 모듈 레벨 플래그로 "한 번" 을 보장한다. 네이티브에서 앱을 다시 켜면 WebView 가
 * 새로 로드되므로 플래그도 초기화된다 — 즉 태블릿을 껐다 켤 때마다 다시 매장
 * 화면으로 들어간다. 반면 앱을 쓰는 도중 사용자가 직접 홈으로 이동한 경우는
 * 다시 튕겨내지 않는다.
 *
 * "앱 실행당 한 번" 이 아니라 "사용자당 한 번" 인 이유는 landingGate 주석 참고 —
 * 로그인 직후에도 동작해야 하기 때문이다.
 */
let landedForUser: string | null = null;

/** 테스트에서 상태를 되돌리기 위한 훅 (프로덕션 경로에서는 쓰지 않는다). */
...
export function useNativeLanding(): void {
  ...
  useEffect(() => {
    ...
    landedForUser = userId;
    if (target) navigate(target, { replace: true });
  }, [navigate, location.pathname, profile, isProfileReady, userId]);
}
