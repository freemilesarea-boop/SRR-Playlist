    ...
    <div className="space-y-10 px-4 pb-16 pt-6 sm:space-y-12 sm:px-6 sm:pt-10 lg:space-y-14 lg:px-8 lg:pt-12">
      ...
      {recents.length > 0 && (
        <PlaylistRow_
          ...
          counts={counts}
          covers={covers}
          seeAllHref="/playlists?view=recent"
        />
      )}
      ...
      {slotPlaylists.length > 0 && (
        <PlaylistRow_
          ...
          counts={counts}
          covers={covers}
          seeAllHref={`/playlists?view=time&slot=${slot}`}
        />
      )}
      ...
      {popular.length > 0 && (
        <PlaylistRow_
          ...
          counts={counts}
          covers={covers}
          seeAllHref="/playlists?view=popular"
        />
      )}
      ...
      {popularByFollows.length > 0 && (
        <PlaylistRow_
          ...
          counts={counts}
          covers={covers}
          seeAllHref="/playlists?view=followers"
        />
      )}
      ...
      {businessPicks.length > 0 && (
        <PlaylistRow_
          ...
          counts={counts}
          covers={covers}
          seeAllHref="/playlists?view=business"
        />
      )}

      {/* 업종별 추천 — AI 자동 생성 플레이리스트 (0113) */}
      {autoGenerated.length > 0 && (
        <PlaylistRow_
          title="✨ 업종별 추천"
          subtitle="업종·시간대·분위기 기반 자동 생성"
          playlists={autoGenerated}
          counts={counts}
          covers={covers}
          seeAllHref="/playlists?view=auto"
        />
      )}
      ...
      {curatorMade.length > 0 && (
        <section className="space-y-3">
          <div className="flex items-end justify-between px-0.5">
            ...
            <Link
              to="/playlists?view=curator-made"
              className="inline-flex items-center gap-0.5 text-xs font-semibold text-accent hover:underline"
            >
              모두 보기 <ChevronRight size={12} />
            </Link>
      ...
      {newest.length > 0 && (
        <PlaylistRow_
          ...
          counts={counts}
          covers={covers}
          seeAllHref="/playlists?view=newest"
        />
      )}
