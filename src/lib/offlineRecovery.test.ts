...
/* ══════════════════════════════════════════════════════════════════════════ */
/* 13A §4 — onOnline 회귀 감사: "멀쩡한 캐시 재생을 되감지 않는다" 를 규칙으로 고정 */
/* ══════════════════════════════════════════════════════════════════════════ */

/**
 * Player 의 onOnline 판정을 순수 함수로 옮겨 적은 것.
 * 실제 코드는 같은 조건을 인라인으로 쓴다 — 여기서 규칙 자체를 못 박는다.
 */
function shouldReloadOnReconnect(i: {
  hasAudio: boolean;
  paused: boolean;
  audioActive: boolean;
...
describe('§4 onOnline 회귀 — 멀쩡한 캐시 재생은 절대 되감지 않는다', () => {
  it('캐시로 정상 재생 중이면 아무것도 하지 않는다', () => {
    expect(shouldReloadOnReconnect({
      hasAudio: true, paused: false, audioActive: true,
      progressedAfterProbe: true, trackChanged: false,
    })).toBe('never');
  });

  it('audioActive=true 인데 실제로는 무음(false positive)이면 1.5초 뒤 복구한다', () => {
    // 숙대점 유형: paused=false, pause 이벤트도 안 와서 플래그가 true 로 남는다.
    expect(shouldReloadOnReconnect({
      hasAudio: true, paused: false, audioActive: true,
  ...
  it('paused 로 굳었으면 즉시 복구한다', () => {
    expect(shouldReloadOnReconnect({
      hasAudio: true, paused: true, audioActive: true,
      progressedAfterProbe: false, trackChanged: false,
    })).toBe('immediate');
  });

  it('소리가 난 적이 없으면(audioActive=false) 즉시 복구한다 — src 없음/preload 상태 포함', () => {
    expect(shouldReloadOnReconnect({
      hasAudio: true, paused: false, audioActive: false,
      progressedAfterProbe: false, trackChanged: false,
    })).toBe('immediate');
  });

  it('audio element 가 아직 없으면 즉시 경로 — 내부 가드가 다시 거른다', () => {
    expect(shouldReloadOnReconnect({
      hasAudio: false, paused: false, audioActive: false,
      progressedAfterProbe: false, trackChanged: false,
    })).toBe('immediate');
  });

  it('확인하는 사이 곡이 넘어갔으면 손대지 않는다 — 새 곡을 되감으면 안 된다', () => {
    expect(shouldReloadOnReconnect({
      hasAudio: true, paused: false, audioActive: true,
      progressedAfterProbe: false, trackChanged: true,
    })).toBe('never');
  });

  it('진행 확인 없이 곧바로 되감는 경로는 존재하지 않는다', () => {
    // audioActive 이면서 곡이 그대로일 때, 즉시(immediate) 복구가 나오는 조합은 없다.
