-- 0190 — 전체 재검수 finalize 가 metadata_cleanup_required(수동 정리/재분류 대상) 플래그를 지우지 않도록 보존.
...
begin
  ...
  loop
    ...
    if v_blocked_declared then
      insert into public.track_rereview_flags(track_id, flag_type, reason) values (r.id,'guardrail_hard','등록한 매장에서 금지규칙 hard_block')
    ...
    if (r.qpass is false) or (r.lufs is not null and (r.lufs < -14 or r.lufs > -10)) then
      insert into public.track_rereview_flags(track_id, flag_type, reason) values (r.id,'quality_review_required', format('LUFS %s / 품질검사', r.lufs))
      on conflict (track_id, flag_type) do nothing; v_flags := v_flags + 1;
    end if;
    if v_blocked_declared or r.ms >= 0.5 then
      insert into public.track_rereview_flags(track_id, flag_type, reason) values (r.id,'high_risk', format('mismatch %s / 등록매장차단 %s', round(r.ms,2), v_blocked_declared))
      on conflict (track_id, flag_type) do nothing; v_flags := v_flags + 1;
    end if;
    if r.release_status='released' and (v_blocked_declared or r.qpass is false) then
      insert into public.track_rereview_flags(track_id, flag_type, reason) values (r.id,'needs_re_review','공개곡 — 등록매장 충돌/품질 재검수 필요(자동 비공개 안 함)')
      on conflict (track_id, flag_type) do nothing; v_flags := v_flags + 1;
    end if;
  end loop;
