-- 0281 — 정산 함수에 eligible_for_payout 필터 복구 (X6.0.1 CRITICAL fix)
--
-- 문제: 0276 anti-abuse 가 admin_generate_monthly_settlement 를 CREATE OR REPLACE 하면서
--       0079 에서 추가했던 `eligible_for_payout=true` 필터를 누락.
--       결과: 자가 재생 / admin 페이지 미리듣기 / 미공개 트랙 재생 등 비정상 스트림이
--             정산에 포함되어 정산금 과다 산정.
--
-- 60일 실측:
--   raw milestone: 1,465
--   is_effective=true: 570 (39%)
--   eligible_for_payout=true: 116 (8%)
--   둘 다 true (= 정확한 정산 대상): 116
--   is_effective=true but NOT eligible: 454 ← 0276 후 잘못 정산에 포함됨
--
-- 수정: 모든 stream_events 카운트에 두 필터 모두 추가:
--   `event_type='milestone_30s' AND is_effective=true AND eligible_for_payout=true`
--
-- 의미:
--   - is_effective: 어뷰징 cap (0275 daily user×track 3회)
--   - eligible_for_payout: 정산 자격 (0078 자가재생/admin/미공개 제외)
--   둘 다 필요하며 의미가 다름.
--
...
begin
  ...
  -- 0281: is_effective AND eligible_for_payout 둘 다 필수
  ...
  return jsonb_build_object(
    ...
    'filters_applied', jsonb_build_array('is_effective', 'eligible_for_payout'),
    'note', 'streams = milestone_30s AND is_effective=true AND eligible_for_payout=true (0281)'
  );
end; $$;
