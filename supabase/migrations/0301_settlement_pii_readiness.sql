-- 0301 — 정산 생성에 PII readiness 통합 (X6.15)
--
-- admin_generate_monthly_settlement 수정:
--   artist_payout_account_ready(user_id) = false 인 아티스트는
--   settlement 를 생성하되 status='held', held_reason='pii_incomplete'
--   로 자동 보류. 정산 항목 (settlement_items) 도 생성해서 다음 달 carry-over
--   기록 보존.
--
-- 안전: 기존 로직 변경 최소화. PII ready 인 아티스트는 동작 동일.

create or replace function public.admin_generate_monthly_settlement(
  p_month date, p_dry_run boolean default true
) returns jsonb
...
begin
  ...
  loop
    ...
    -- ✅ X6.15: PII readiness 체크 — 미완료면 held 로 자동 보류
    ...
    -- ✅ X6.15: PII 미완료 또는 verified 계좌 없음 → status='held'
    if not v_pii_ready then
      v_final_status := 'held';
      v_final_held_reason := 'pii_incomplete';
      v_held_count := v_held_count + 1;
    else
      v_final_status := 'pending';
      v_final_held_reason := null;
    end if;
    ...
    if not p_dry_run then
      if v_existing.id is not null then
        ...
        update public.artist_settlements set
          ...
          held_reason = v_final_held_reason
  ...
  return jsonb_build_object(
    ...
    'note', 'X6.15: PII 미완료 시 status=held / held_reason=pii_incomplete 로 자동 보류'
  );
end; $$;
