-- 0165 — 매장 프로파일 목록 + 플레이리스트 store_key 지정 + 큐레이션 필터 확장
create or replace function public.admin_list_store_profiles()
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object('store_key',store_key,'store_label',store_label) order by sort_order),'[]'::jsonb) into v
  from public.ai_store_profiles where active;
  return v;
end; $$;
grant execute on function public.admin_list_store_profiles() to authenticated;

create or replace function public.admin_set_playlist_store_key(p_playlist_id uuid, p_store_key text)
returns jsonb language plpgsql security definer set search_path to 'public' as $$
begin
  if not exists (select 1 from public.users u where u.id=auth.uid() and u.role='admin') then raise exception 'admin only'; end if;
  if p_store_key is not null and not exists (select 1 from public.ai_store_profiles where store_key=p_store_key) then
    raise exception 'unknown store_key %', p_store_key;
  end if;
  update public.playlists set ai_store_key = nullif(btrim(p_store_key),'') where id=p_playlist_id;
  return jsonb_build_object('ok', true);
end; $$;
grant execute on function public.admin_set_playlist_store_key(uuid, text) to authenticated;
